...
INSERT INTO test_numeric2
VALUES ( 1234567890,
'Точность 10 знаков, масштаб 0 знаков (целое число)' ); --2

select * from test_numeric2

SELECT 'NaN'::numeric > 10000; --3

SELECT '5e-324'::double precision > '4e-324'::double precision;

SELECT '5e-324'::double precision;

SELECT '4e-324'::double precision; --4


SELECT '1E-37'::real > '1E+37'::real; --4.1

SELECT 'Inf'::double precision > 1E+308; --5
SELECT 'Inf'::double precision > 1E-307; --5.1

SELECT 0 * 'Inf'::real; --6

select 'NaN'::real > 'Inf'::real; --6.1
...
INSERT INTO test_serial ( name ) VALUES ( 'Луговая' ); --7
...
DELETE FROM test_serial2 WHERE id = 4; --8

SELECT current_time;

SELECT current_time::time( 3 );
SELECT current_timestamp::timestamp( 3 );
--SELECT current::interval( 3 ); ???
--11	
...
SELECT to_char(CURRENT_date, text) --15	

SELECT 'Feb 25, 2015'::date; --16

SELECT '21:15:16'::time;
SELECT '21:15:16:23'::time; --17


SELECT ( '2016-09-16'::date - '2016-09-01'::date ); --18
SELECT ( '20:34:35'::time - '19:44:45'::time ); --19

SELECT ( current_timestamp - '2016-01-01'::timestamp )
AS new_date;
SELECT ( current_timestamp + '1 mon'::interval ) AS new_date; --20

SELECT ( '2016-01-31'::date + '1 mon'::interval ) AS new_date; --29.02.2016
SELECT ( '2016-02-29'::date + '1 mon'::interval ) AS new_date; --21

SHOW intervalstyle; --22

SELECT ( '2016-09-16'::date - '2015-09-01'::date );
SELECT ( '2016-09-16'::timestamp - '2015-09-01'::timestamp ); --23

SELECT ( '20:34:35'::time - '00:01:00' ); --modified
SELECT ( '2016-09-16'::date - 1 ); --24

SELECT ( date_trunc( 'week', interval '1999-11-27 12:34:56.987654' ) ); --25

-- date_trunc - отсекает строку до заданной точности --26

SELECT extract('century' from interval '2101-11-27 12:34:56.123459'); 
-- extract - выделяет заданный параметр (например день, секунда и тд) --27

SELECT extract(month from interval '2 years 3 months') --28
...
SELECT * FROM databases WHERE is_open_source <> 1; --НЕ ЯВЛЯЮТСЯ. ЗНАК <> ИМЕЕТ ЗНАЧЕНИЕ ПРОТИВОПОЛОЖНОСТИ или же КРОМЕ.
--29
...
SELECT array_cat( ARRAY[ 1, 2, 3 ], ARRAY[ 3, 5 ] );  --объединение массивов

SELECT array_remove( ARRAY[ 1, 2, 3 ], 3 ); -- удаление из массива

SELECT array_length(array[1,2,3], 1) --длина
--32
...
WHERE pilot_name = 'Boris'; --34

SELECT '{ "sports": "хоккей" }'::jsonb || '{ "trips": 5 }'::jsonb; --35
